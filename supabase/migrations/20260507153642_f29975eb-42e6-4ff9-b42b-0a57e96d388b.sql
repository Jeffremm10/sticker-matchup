...
CREATE OR REPLACE FUNCTION public.get_likes_received()
 RETURNS TABLE(anon_id text, distance_km integer, is_pro boolean, receive_count bigint, give_count bigint, receive_ids integer[], give_ids integer[], liker_id uuid, liker_display_name text)
 LANGUAGE plpgsql
...
DECLARE
  ...
  _reveal boolean;
BEGIN
  SELECT p.is_pro, p.tier INTO _me_pro, _me_tier FROM public.profiles p WHERE p.id = _me;
  _reveal := COALESCE(_me_pro, false) OR COALESCE(_me_tier, 'free') = 'premium';
  ...
  SELECT
    ...
    CASE WHEN _reveal THEN l.uid ELSE NULL END AS liker_id,
    CASE WHEN _reveal THEN p.display_name ELSE NULL END AS liker_display_name
  FROM likers l
